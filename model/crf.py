...
def tensor_to_array(tensor):
    array = np.nan * np.zeros(tensor.shape)
    if len(array.shape) != 2:
        raise Exception('must 2!')
    for i in range(array.shape[0]):
        for j in range(array.shape[1]):
            array[i, j] = float(tensor[i, j])
    return array
...
def log_sum_exp(vec, m_size):
    ...
    """
    # [gcq] 输入vec的size是(batch_size, tag_size, tag_size)
    _, idx = torch.max(vec, 1)  # B * 1 * M  # [gcq] 在维度1上求最大值，维度1的长度被聚合掉（维度编号从0开始，维度1是第2个维度。）
    max_score = torch.gather(vec, 1, idx.view(-1, 1, m_size)).view(-1, 1, m_size)  # B * M  # [gcq] 把上面找到的最大值归集到一起
    return max_score.view(-1, m_size) + torch.log(torch.sum(torch.exp(vec - max_score.expand_as(vec)), 1)).view(-1, m_size)  # B * M  # [gcq] expand_as，把max_score扩展为和vec一样的维度，方便进行逐元素的运算
...
class CRF(nn.Module):
    ...
    def __init__(self, tagset_size, gpu):
        ...
        init_transitions = torch.zeros(self.tagset_size+2, self.tagset_size+2)  # [gcq] 构造一个全0的tensor。这个变量应该是类似于HMM中的转换矩阵的概念，猜测加2可能是人为加入了句子的“起始标签”和“结束标签”。
        # [gcq] 对tensor用负数索引，效果跟numpy的负数索引是一样的，即-1表示索引到最后一个元素，-2表示索引到倒数第2个元素。。。
        init_transitions[:,START_TAG] = -10000.0  # [gcq] 表示从任何标签转换到起始标签，都会导致该局部得分为-10000
        init_transitions[STOP_TAG,:] = -10000.0  # [gcq] 表示结束标签转换到任何标签，都会导致该局部得分为-10000
        init_transitions[:,0] = -10000.0
        init_transitions[0,:] = -10000.0
        if self.gpu:
            init_transitions = init_transitions.cuda()
        # [gcq] nn.Parameter是Tensor的一个子类，如果一个nn.Parameter对象成为nn.Module对象的属性，它会自动被加入到模型的参数列表里面。
        self.transitions = nn.Parameter(init_transitions) # [gcq] 整个crf模型只有转换矩阵这一个参数？？？不解？？__init__里面没有再看到有其他参数被构造了。
    ...
    def _calculate_PZ(self, feats, mask):
        """
            input:
                ...
                masks: (batch, seq_len)
        """
        # [gcq] 这里的feats可能对应的是crf中的特征函数（的值），参见这里对特征函数的定义 https://zhuanlan.zhihu.com/p/26696451?group_id=843138862132895744
        # [gcq] 这里输入的feats是wordsequence层的输出
        batch_size = feats.size(0)
        seq_len = feats.size(1)
        tag_size = feats.size(2)
        # print feats.view(seq_len, tag_size)
        assert(tag_size == self.tagset_size+2)
        mask = mask.transpose(1,0).contiguous() # [gcq] tensor在内存中可能不是连续的，contiguous方法把tensor内的数据复制到内存中一块连续的地方，然后返回这个连续的tensor
        ins_num = seq_len * batch_size
        ## be careful the view shape, it is .view(ins_num, 1, tag_size) but not .view(ins_num, tag_size, 1)
        # [gcq] transpose，对tensor进行转置，两个参数是需要互换的维度
        # [gcq] view，相当于reshape，对tensor的形状进行重构，
        # [gcq] expand，将tensor的某一个维度扩展。
        feats = feats.transpose(1,0).contiguous().view(ins_num,1, tag_size).expand(ins_num, tag_size, tag_size)
        ## need to consider start
        # [gcq] 下面这里相加是对feats里面的得分作一些基本的修正，把某些不符合基本原则的地方的得分减掉10000，即加入惩罚项，见__init__里面的注释。
        scores = feats + self.transitions.view(1,tag_size,tag_size).expand(ins_num, tag_size, tag_size)
        scores = scores.view(seq_len, batch_size, tag_size, tag_size)
        # build iter
        seq_iter = enumerate(scores)  # [gcq] 只对第一个维度，即seq_len这个进行迭代，迭代取出的元素的size是(batch_size, tag_size, tag_size)
        ...
        for idx, cur_values in seq_iter: # [gcq] 此处的循环是从第二个元素开始，因为上面已经对seq_iter取过一次next了
            # previous to_target is current from_target
            # partition: previous results log(exp(from_target)), #(batch_size * from_target)
            # cur_values: bat_size * from_target * to_target

            # 下面这里为什么要view？partition的shape不是本来就是这样的吗？
